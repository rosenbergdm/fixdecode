...
XNode* parse_fix_spec_xml(const char* szfile)
{
    ...
    {
        // fprintf(stdout, "ERROR parsing fix spec : %s\n", szfile);
        delete root;
        free(handler);
        return NULL;
    }
    ...
    if (0==root->nods.size())
    {
        // fprintf(stdout, "ERROR empty fix spec : %s\n", szfile);
        delete root;
        free(handler);
        return NULL;
    }
...
int MessageGenerator::msg_bad(const char* sz, int len)
{
    ...
    if (0!=strncmp(sz, lhs.c_str(), lhs.length()))
        ...
        printf("FIX msg, but bad FIX version : expecting %s\n", lhs.c_str());

    lhs = "8=" + prelude + soh;
    if (0!=strncmp(sz, lhs.c_str(), lhs.length()))
        // return printf("FIX msg, but bad delimiter\n"), -1;
        printf("FIX msg, but bad delimiter\n");
    ...
    if (0!=strncmp(sz+len-rhs.length(), rhs.c_str(), rhs.length()))
        ...
        printf("FIX msg, but bad checksum : expecting %s\n", ssum.c_str());
...
int MessageGenerator::show_expanded_spec(const char* szmsgtype, mapss& options)
{
    ...
    if (!xmsg)
    {
        fprintf(stdout, "unknown message type [%s]\n", szmsgtype);
        exit(-1);
    }
